...
module.exports = {
  ...
  async execute(message, client) {
    if (!client.config) {
      return message.reply('Sistem ayarları yüklenemedi!'.red.bold);
    }
    ...
    try {
        ...
        if (interaction.customId === 'unban_all') {
          ...
          for (const [id] of allBans) {
            try {
              ...
              console.error(`Ban kaldırma hatası (${id}):`.red.bold, error);
            }
          }
      ...
      console.error('Ban listesi hatası:'.red.bold, error);
      await message.reply('Ban listesi alınırken hata oluştu!'.red.bold);
    }
  }
};